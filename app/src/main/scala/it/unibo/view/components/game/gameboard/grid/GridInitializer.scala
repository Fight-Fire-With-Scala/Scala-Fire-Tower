...
class GridInitializer(gridSize: Int, squareSize: Double, handleCellHover: (Int, Int, HoverDirection) => Unit, handleCellClick: () => Unit) {
  val squareMap: mutable.Map[Position, GridSquare] = mutable.Map()

  def initializeGridSquares(gridPane: GridPane): Unit = {
    for {
