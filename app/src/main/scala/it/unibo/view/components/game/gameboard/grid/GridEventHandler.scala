...
import it.unibo.view.components.game.gameboard.grid.FigurePattern.{Row3, SingleCell, Void}
...
class GridEventHandler(
  ...
  def handleCellClick(row: Int, col: Int, gamePhase: GamePhase): Unit =
    ...
    gamePhase match
      ...
      case PlayCardPhase    => if (hoveredCells.contains(position))
          val pattern = availablePatterns.find(_.contains(position)).get
  ...
  private def hoverForClickedCells(row: Int, col: Int, hoverDirection: HoverDirection): Unit =
    ...
    }.map(_.keySet).distinct.flatMap(keys => availablePatterns.filter(_.keySet == keys))
  ...
  def handleCellHover(
    ...
    case GamePhase.PlayCardPhase    =>
      if clickedCells.isEmpty then hoverForAvailablePatterns(row, col)
