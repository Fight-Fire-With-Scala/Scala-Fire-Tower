...
class HandComponent extends GraphicComponent:
  ...
  @FXML
  def initialize(): Unit =
    cardComponents = handPane.getChildren.asScala.toList.map(_ => None)

  def setupCard(cardPane: Node, cardComponent: CardComponent, slotIndex: Int): Unit = Platform.runLater { () =>
