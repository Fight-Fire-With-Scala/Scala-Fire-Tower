...
import it.unibo.model.cards.effects.{f, pattern, w}
...
case object WaterCard:
  ...
  case object SmokeJumper extends WaterCard:
    ...
    val effect: SpatialResolver =
      PatternResolver(pattern { w; w; w; w; f; w; w; w; w }.toMap(3, 3))

  case object AirDrop extends WaterCard:
    val effectCode: Int = 12
    val effect: SpatialResolver = PatternResolver(pattern { w; w; w }.toMap(1, 3))

  case object FireEngine extends WaterCard:
    val effectCode: Int = 13
    val effect: SpatialResolver = PatternResolver(pattern { w; w; w; w }.toMap(2, 2))
