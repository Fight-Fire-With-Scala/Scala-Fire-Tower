...
sealed trait PatternType:
  ...
  private def compilePattern: Map[Position, Token] = PatternType.fillPattern(template, tokens)
...
object PatternType:
  ...
  final case class VeryLargeEffect(tokens: Map[String, Token]) extends PatternType:
    override val template: Map[Position, Token] = pattern { a; a; a; a; b; a; a; a; a }.mapTo(3, 3)

  final case class LargeEffect(tokens: Map[String, Token]) extends PatternType:
    override val template: Map[Position, Token] = pattern { a; a; a; a }.mapTo(2, 2)

  final case class MediumEffect(tokens: Map[String, Token]) extends PatternType:
    override val template: Map[Position, Token] = pattern { a; a; a }.mapTo(1, 3)

  final case class MediumAltEffect(tokens: Map[String, Token]) extends PatternType:
    override val template: Map[Position, Token] = pattern { a; b; a }.mapTo(1, 3)

  final case class SmallEffect(tokens: Map[String, Token]) extends PatternType:
    override val template: Map[Position, Token] = pattern { a; a }.mapTo(1, 2)

  final case class VerySmallEffect(tokens: Map[String, Token]) extends PatternType:
